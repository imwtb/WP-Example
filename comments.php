...
<div id="comments" class="comments__area">
	...
	<?php else : ?>
		...
		comment_form([
			...
			'must_log_in'          => '<p class="comment__mustlogin"><a href="' . wp_login_url(apply_filters('the_permalink', get_permalink($id), $id)) . '">' . __('登录', 'imwtb') . '</a>' . __('后发表评论！', 'imwtb') . '</p>',
			'comment_notes_before' => '<p class="comment__notes">' . __('您的电子邮件地址不会被公开。', 'imwtb') . $text . '</p>',
			'logged_in_as'         => '<p class="comment__loggedinas"><a href="' . get_edit_user_link() . '">' . __('登录为', 'imwtb') . $name . '</a>. <a href="' . wp_logout_url(apply_filters('the_permalink', get_permalink($id), $id)) . '">' . __('退出?', 'imwtb') . '</a>' . $text . '</p>',
